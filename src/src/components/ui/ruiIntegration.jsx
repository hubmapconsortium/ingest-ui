...
class RUIIntegration extends Component {
 ...
 // console.debug('%c◉ organMap ', 'color:#9000FF', organMap);
  ...
  updateRUIConfig() {
    let fullOrgans = JSON.parse(localStorage.getItem("organs_full"));
    let term = this.props.organList[this.props.organ];
    let organDetails = fullOrgans?.find((org) => org.term === term) || null;
    let uberonUrl = organDetails?.uberon_url;
    const ontologyId = uberonUrl
