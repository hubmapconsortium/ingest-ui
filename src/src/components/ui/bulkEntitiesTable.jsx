...
import {
  ParseRegErrorFrame,
  parseErrorMessage,
  TableErrorRowProcessing,
} from "../../utils/error_helper.jsx";
...
export function BulkEntitiesTable({
  type,
  onDataChange,
  setBulkRegistrationMessage,
}) {
  ...
  let docs =
    "https://docs.hubmapconsortium.org/bulk-registration/" +
    type.toLowerCase() +
    "-bulk-reg.html";
  ...
  const columnsSuccess = type
    ? colSuccessSelection(type)
    : COLUMN_DEF_BULK_SAMPLES;
  ...
  function handleFileGrab(e) {
    dimSpotlight();
    // - [autoSHH] console.debug('%c◉ Grabbing file ', 'color:#008000');
    ...
    if (newFile && newFile.name.length > 0) {
      ...
      Papa.parse(newFile, {
        ...
        header: true,
        complete: (data) => {
          // - [autoSHH] console.debug('%c◉ data ', 'color:#008000', data.data);
          ...
          let matchingCols = fileCols.filter((col) =>
            expectedCols.includes(col),
          );
          ...
          // - [autoSHH] console.debug('%c◉ fileData ', 'color:#008000', fileData, fileData.rows);
        },
      });
    } else {
    }
  ...
  useEffect(() => {
    if (
      bulkEntityValidationErrors?.some((error) => error?.row && error?.column)
    ) {
      window.setTimeout(
        () => highlightTableErrors(bulkEntityValidationErrors),
        0,
      );
    } else if (
      !bulkEntityValidationErrors ||
      bulkEntityValidationErrors.length === 0
    ) {
      highlightTableErrors("clear");
    }
  }, [bulkEntityValidationErrors]);
    ...
    ingest_api_bulk_entities_upload(type + "s", newFile)
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
          ...
          if (
            res?.res?.response?.data?.code === 406 &&
            typeof res?.res?.response?.data?.description?.description ===
              "string"
          ) {
            // THis might just be a metadata issue
            let parsedPreflight = ParsePreflightString(
              decodeURI(errorSet?.description),
            );
            let errString;
            let name = res?.res?.response?.data?.description?.name
              ? res?.res?.response?.data?.description?.name
              : null;
          ...
          // - [autoSHH] console.debug('%c◉ "Please Review the following validation errors and re-upload your file." ', 'color:#008000', );
          setPageErrors((prevValues) => ({
            ...prevValues,
            bulkEntity:
              "Please Review the following validation errors and re-upload your file.",
          }));
        } else if (res?.error?.response?.data?.error) {
          // 400 / too many
          // - [autoSHH] console.debug('%c◉ 400! ', 'color:#008000', res?.error?.response?.data?.error );
          const errorData = {
          ...
          setPageErrors((prevValues) => ({
            ...prevValues,
            bulkEntity:
              "An error occurred during file upload. Please review the message and try again. || " +
              res.toString(),
          }));
        }
        //setValidatingBulkEntityUpload(false)
        ...
        if (userGroups.length > 1 && showGroupCheck === true) {
          // - [autoSHH] console.debug('%c◉ SHOWING ', 'color:#008000', );
          setLoaders((prev) => ({ ...prev, showGroupSelect: true }));
        }

        let fullTableContent = document.getElementsByClassName(
          "MuiDataGrid-virtualScrollerRenderZone",
        );
        let newHeight = fullTableContent[0].clientHeight;
        let outerTable = document.getElementsByClassName("HDTdynamic");
        if (newHeight > 350) {
          logger.debug("BulkEntititiesTable.handleFileUpload", newHeight);
          newHeight = 300;
        }
        outerTable[0].setAttribute(
          "style",
  ...
  function handleRegister() {
    // - [autoSHH] console.debug('%c◉ HANDLE REGISTER ', 'color:#008000', fileData );

    setLoaders((prev) => ({ ...prev, registration: true }));
    let fileRef = {
      temp_id: fileData.temp_id,
      group_uuid: fileData.group
        ? fileData.group
        : JSON.parse(localStorage.getItem("userGroups"))[0]?.uuid || "",
    };
    // - [autoSHH] console.debug('%c◉ fileRef ', 'color:#008000', fileRef);
    try {
      ingest_api_bulk_entities_register(type, fileRef)
        .then((resp) => {
          ...
          // - [autoSHH] console.debug('%c◉ serverResp ', 'color:#008000', serverResp);
          if (resp.status) {
            if (resp.status === 202) {
              ...
              if (responseKeys.length) {
                const messageValue =
                  serverResp.results[responseKeys[0]].split("=");
                const batchId =
                  messageValue.length > 1 ? messageValue[1].trim() : null;
                const body = <span>{responseKeys[0]}.</span>;
                setBulkRegistrationMessage({
                  body,
                  batchId,
                  message: responseKeys[0],
                });
              }
              ...
              logger.all.error(errorData);
              setBulkRegistrationMessage({
                body: "An error occured with this request",
                status: "error",
                message: `${resp.status}`,
              });
            }
            ...
            (resp.error &&
              resp.error.response &&
              resp.error.response.status === 504)
          ) {
            ...
            if (
              ...
              // - [autoSHH] console.debug('%c◉ errorArray ', 'color:#008000', errorArray);
              ...
              var grabFullError = {
                ...
                data: serverResp.data.data,
              };
              setPageErrors({
                status: "server",
                message: "Registration Failure",
                detail: grabFullError,
              });
            ...
            const errRows = entries
              ...
              .map(([key, val], i) => {
                ...
                const originalRow = bulkEntityRows[rowIndex - 1]
                  ? bulkEntityRows[rowIndex - 1]
                  : {};
            ...
            // - [autoSHH] console.debug('%c◉ errRows ', 'color:#008000', errRows);
            // Keep success rows intact (original response objects)
            const successRows = entries
              .filter(([, val]) => !(val && val.error))
              .map(([, val]) => val);
            // - [autoSHH] console.debug('%c◉ errRows ', 'background:#008000', errRows);
            // - [autoSHH] console.debug('%c◉ successRows ', 'background:#008000', successRows);
  ...
  function highlightTableErrors(errorSet) {
    ...
    if (errorSet && errorSet.length > 0 && errorSet !== "clear") {
      ...
      for (const error of errorSet) {
        let errorRow = document.querySelector(
          `.associatedBulkEntityTable [aria-rowindex="${error.row}" ]`,
        );
  ...
  function spotlightCellAndRow(e, error, target) {
    // Turn off Old Lights
    document
      .querySelector(`[data-spotlight="true" ]`)
      ?.removeAttribute("data-spotlight");
    ...
    let spotlightTargets = Array.from(
    ...
    const hasUndefinedErrRow = cleanTargets.some(
    ...
    setTimeout(() => {
      cleanTargets.forEach((el) =>
        el?.removeAttribute("data-spotlight", "true"),
      );
    }, 4000);
  }
  ...
  function setSelectionTableRow(e, item, target) {
    // - [autoSHH] console.debug('%c◉ setSelectionTableRow ', 'color:#008000', setSelectionTableRow);
    ...
    let dataGridContainer = document.querySelector(
      ".associatedBulkEntityTable",
    );
    let selectedRow = dataGridContainer?.querySelector(
  ...
  function setSelectionListRow(e, item, target) {
    // - [autoSHH] console.debug('%c◉ setSelectionListRow ', 'color:#008000', e, item, target );
    let oldSelected = document.querySelectorAll("[data-selected]");
    oldSelected.forEach((el) => el.removeAttribute("data-selected"));
    oldSelected.forEach((el) => el.classList.remove("Mui-selected"));
    // we only wanna run this if we're not being selected by the row already, check event
    // - [autoSHH] console.debug('%c◉ setSelectionListRow e ', 'color:#008000', e);
      ...
      <Box className="HDT errorFrame mt-4">
          ...
          {!fileData.registered && (
            <>
              <FontAwesomeIcon
                ...
                color="red"
                className="mr-2 errIcon red"
              />
              <Typography className="preamble" variant="overline">
                The following errors were encountered when trying to upload your
                data. Please review the messages below and try again.
              </Typography>
            </>
          )}
          {bulkEntityRows && bulkEntityRows.length <= 0 && (
            ...
            >
              <FontAwesomeIcon
                ...
                className="mr-2 red"
              />{" "}
              Additionally, no valid columns were found in the uploaded file, so
              no data can be displayed. Please ensure your file matches the
              expected format.
            </Typography>
          )}
          {fileData.registered && (
            <Typography className="preamble" variant="caption">
              <FontAwesomeIcon
                ...
                className="mr-2 red"
              />
              &nbsp;The following rows were not accepted; Please review
              validation messages and try again.
            </Typography>
          )}
        </Box>
        <Box
          className="errorListWrap"
        ...
        >
          <ErrorList
            ...
            onHover={({ event, item }) => {
              const col =
                item?.column === "organ_type" ? "organ" : item?.column;
              spotlightCellAndRow(
                event,
    ...
    return (
      <Box className="pagelessFooter">
        <Typography variant="caption">
          {" "}
          <strong>Total Rows:</strong> {props.rowCount.toString()}
        </Typography>
      </Box>
    );
  }
  ...
  function renderEntityTable() {
    return (
      <>
        <div
          className={"associationTableWrap associatedBulkEntityTable"}
          style={{ width: "100%" }}
        >
          <DataGrid
            apiRef={apiRef}
        ...
        {fileData?.uploaded &&
          !fileData.registered &&
          bulkEntityValidationErrors &&
          bulkEntityValidationErrors.length > 0 && <>{renderErrorFrame()}</>}
        <Box className="">
          <Typography variant="caption">
            Please refer to the{" "}
            <a href={docs} target="_blank" rel="noreferrer">
              Bulk {type} file schema information
            </a>
            , and this{" "}
            <a
              href={`https://raw.githubusercontent.com/hubmapconsortium/ingest-ui/main/src/src/assets/Documents/example-${type.toLowerCase()}-registrations.tsv`}
              target="_blank"
              rel="noreferrer"
            >
              Example TSV File
  ...
  function CustomToolbarExportAllRows() {
    ...
    const apiInst =
      apiRefLocal && apiRefLocal.current ? apiRefLocal.current : apiRefLocal;
    ...
    return (
      <Toolbar aria-label="Bulk registration table controls">
        ...
        <ToolbarButton
          ...
          render={
            <Button size="small" sx={{ ml: 1 }} startIcon={<SaveAltIcon />} />
          }
        >
          Export All
        </ToolbarButton>
    ...
    return (
      <Box>
        <Box className="successAlertWrap">
          ...
          <Alert
            className="mt-4 mb-0 successAlert"
            sx={{ borderRadius: "4px 4px 0px 0px" }}
          >
            <Box>
              <strong>Success:</strong> The following rows registered
              successfully!
            </Box>
          </Alert>
        </Box>
        <div
          className={
            "associationTableWrap associatedBulkEntityTable successWrap"
          }
          style={{ width: "100%" }}
        >
          <DataGrid
            ...
            hideFooterSelectedRowCount
            rowCount={
              successRows && successRows.length > 0 ? successRows.length : 0
            }
            sx={{
              fontSize: "0.75em",
              border: "none",
    ...
    return (
      <>
        <Alert
          ...
          sx={{ borderRadius: "4px 4px 0px 0px" }}
        >
          <strong>Errors:</strong> The following rows were not registered.
          Please review the provided error messaging and try again.
        </Alert>
        <div
          className={
            "associationTableWrap associatedBulkEntityTable errorSetWrap regErrorTable mt-0 w-100"
          }
        >
          <DataGrid
  ...
  function calcRegDisabled() {
    let erCheck =
      bulkEntityValidationErrors && bulkEntityValidationErrors?.length > 0;
  ...
  return (
    <>
      {/* Main Upload Table */}
      {!fileData?.registered && (
        <FormControl className="w-100">{renderEntityTable()}</FormControl>
      )}

      {/* Reg Success Table */}
      {fileData?.uploaded &&
        fileData?.registered &&
        fileData?.regValidation?.success &&
        fileData?.regValidation?.success.length > 0 && (
          <>
            <FormControl className="w-100">{renderSuccesTable()}</FormControl>
          </>
        )}
      ...
      {fileData?.uploaded &&
        fileData?.registered &&
        fileData?.regValidation?.error &&
        fileData?.regValidation?.error.length > 0 && (
          <FormControl className="w-100">{renderRegErrorTable()}</FormControl>
        )}
      ...
      <Box
        ...
        sx={{ display: "inline-block", width: "100%", mt: 2 }}
      >
            ...
            <Collapse in={loaders.showGroupSelect}>
              <FormControl size="small">
                ...
                <Select
                  ...
                  label="Group"
                  onChange={(e) =>
                    setFileData((prev) => ({ ...prev, group: e.target.value }))
                  }
                  sx={{
