...
export const FormHeader = (props) => {
  ...
  let details =
    props.entityData[0] !== "new"
      ? `${entityData.entity_type}: ${entityData.hubmap_id}`
      : `New ${props.entityData[1]}`;
  let permissions = props.permissions;
  let globusURL = props.globusURL;
  document.title = `HuBMAP Ingest Portal | ${details}`; //@TODO - somehow handle this detection in App
  let entityType = entityData.entity_type
  ...
  return (
    <Grid
      ...
      size={12}
      className="FormHead"
      sx={{ marginBottom: "5px", padding: "10px", position: "relative" }}
    >
      {entityData[0] !== "new" && (
        <>
          <Typography
  ...
  return (
    <Box sx={{ width: "100%", display: "flex", marginTop: "20px" }}>
      <Box
        sx={{ marginRight: "20px" }}
        className={
          (permissions.has_admin_priv && entityData.status === "Reorganized") ||
          permissions.has_admin_priv === false
            ? "col-6 taskAssignment disabled"
            : "col-6"
        }
      >
        <InputLabel htmlFor="ingest_task">Ingest Task</InputLabel>
        <TextField
          id="ingest_task"
          name="ingest_task"
          ...
          slotProps={{
            inputLabel: {
              shrink: uuid || formValues?.ingest_task ? true : false,
            },
          }}
      ...
      <Box className="col-6 ">
        ...
        <NativeSelect
          ...
          value={
            formValues.assigned_to_group_name
              ? formValues.assigned_to_group_name
              : ""
          }
        >
...
export function GroupModal({ selectionAction, open, closeGroupModal }) {
  let userGroups = JSON.parse(localStorage.getItem("userGroups")) || [];
  let [selectedGroup, setSelectedGroup] =
    userGroups.find((g) => g.selected)?.displayname || "";
  return (
    <Dialog aria-labelledby="group-dialog" open={open}>
      <DialogTitle>
        You currently have multiple group assignments, Please select a primary
      ...
      <DialogActions>
        ...
        <Button variant="outlined" onClick={(e) => closeGroupModal(e)}>
          Cancel
        </Button>
      </DialogActions>
    </Dialog>
  ...
  return (
    <>
      <Typography variant="caption" color={"#444a65"}>
        <strong>
          <FontAwesomeIcon sx={{ padding: "1.2em" }} icon={faHeadset} />
        </strong>
        If this message persists, please reach out to help@hubmapconsortium.org
        symbol beneith the table will re-launch this message
      </Typography>
    </>
  );
}
...
export function StatusBadge(status) {
  ...
  return (
    <Chip
      sx={{ fontWeight: "bold", fontVariant: "all-small-caps" }}
...
export function NewBadge(type, mini, status = "NEW", label = "NEW") {
  logger.debug("%c◉ NewBadge ", "color:#008000", type);
  let NewBadgeStyle;
  if (mini) {
    NewBadgeStyle = {
  ...
  return (
    <Chip
      style={NewBadgeStyle}
      className={"newBadge " + badgeClass(status)}
      icon={mini ? null : IconSelection(type, "new")}
      label={label}
      size="small"
    />
  );
...
function revisionLinksTime(entityData) {
  logger.debug(
    "%c◉ revisionLinksTime entityData ",
    "color:#008000",
    entityData,
  );
  const fauxHrefStyle = {
    ...
    <>
      {entityData.next_revision_uuid && (
        <Typography
          ...
          onClick={() =>
            window.open(type + "/" + entityData.next_revision_uuid, "_blank")
          }
          variant="caption"
        >
          <UpdateIcon className="iconEffect" sx={{ marginRight: "5px" }} />
          This {entityData.entity_type} has a{" "}
          <strong>
            <Typography className="hoverRise" sx={fauxHrefStyle}>
              {" "}
              next version
            </Typography>{" "}
          </strong>
      ...
      {entityData.previous_revision_uuid && (
        <Typography
          ...
          onClick={() =>
            window.open(
              type + "/" + entityData.previous_revision_uuid,
              "_blank",
            )
          }
        ...
        >
          <UpdateIcon
            ...
            sx={{ transform: "scaleX(-1)", marginRight: "5px" }}
          />
          This {entityData.entity_type} has a{" "}
          <strong>
            <Typography className="hoverRise" sx={fauxHrefStyle}>
  ...
  return entityData[0] !== "new" ? (
    <Grid size={6} className="entityDataHead">
      {entityData.status && (
        <>
          <Typography
            sx={{ width: "auto", float: "left", marginRight: "10px" }}
          >
      ...
      {entityData.priority_project_list && (
        <Typography variant="caption" sx={{ display: "inline-block" }}>
          ...
          {entityData.priority_project_list?.length > 1
      ...
      <Typography
        ...
        sx={{ display: "inline-block", width: "100%" }}
      >
        <strong>Entered by: </strong> {entityData.created_by_user_email}
      </Typography>
      <Typography
        variant="caption"
        sx={{ display: "inline-block", width: "100%" }}
      >
        <strong>Group: </strong> {entityData.group_name}
      </Typography>
      {(entityData.entity_type === "Donor" ||
        entityData.entity_type === "Sample") && (
        <Typography
          variant="caption"
          sx={{ display: "inline-block", width: "100%" }}
        >
          <strong>Submission ID: </strong> {entityData.submission_id}
        </Typography>
      )}
      <Typography
        variant="caption"
        sx={{ display: "inline-block", width: "100%" }}
      >
        <strong>Entry Date: </strong> {tsToDate(entityData.created_timestamp)}
      </Typography>
    </Grid>
  ) : (
    <React.Fragment>
      <Grid
        size={["Upload", "EPICollection"].includes(entityData[1]) ? 9 : 6}
        className=""
      >
        <Typography
          variant="h4"
          sx={{ fontWeight: 300, display: "inline-block", fontSize: "1em" }}
        >
          {" "}
          Registering a&nbsp;
        </Typography>
        {NewBadge(null, true)} <br />
        <Typography
          sx={{ fontWeight: 300, display: "inline-block", fontSize: "1.4em" }}
        >
          {IconSelection(entityType)}
        </Typography>{" "}
        <Typography
          variant="h3"
          sx={{ fontWeight: 300, display: "inline-block", fontSize: "2.4em" }}
        >
          {" "}
          {type}{" "}
        </Typography>
      ...
      {entityData[1] === "Upload" && (
        <Grid size={6} className="">
          <Typography sx={{ marginRight: "10px" }}>
            Register a new Data Upload that will be used to bulk upload data,
            which will be organized by HIVE into multiple datasets. For more
            information about registering and uploading data see the{" "}
            <a
              href="https://docs.hubmapconsortium.org/data-submission/"
              target="_blank"
            >
...
function infoPanels(entityData, permissions, globusURL) {
  ...
  const isEPICollection =
    type === "EPICollection" || String(type).toLowerCase() === "epicollection";
  if (type === "Bulk") {
  ...
  if (
    ...
    (entityData.next_revision_uuid || entityData.previous_revision_uuid)
  ) {
    linkBuffers = "5px";
  }
  ...
  return (
    <Grid
      size={isEPICollection && entityData[0] === "new" ? 3 : 6}
      className=""
    >
      {entityData.creation_action && entityData.creation_action !== "" && (
        <Box>
          <ReturnCreationActionDetail
            creation_action={entityData.creation_action}
          />
        </Box>
      )}
      ...
      {globusURL && (
        ...
        >
          ...
          >
            <a href={globusURL} target="_blank" rel="noopener noreferrer">
              {entityData.status &&
                (entityData.status.toUpperCase() === "REORGANIZED" ||
                  entityData.status.toUpperCase() === "SUBMITTED") && (
      ...
      {permissions.has_write_priv &&
        ((entityData[0] === "new" && HIPPATypes.includes(entityData[1])) ||
          HIPPATypes.includes(entityData.entity_type)) && <HIPPA />}

      {entityData &&
        ((entityData.data_access_level &&
          entityData.data_access_level === "public") ||
          (entityData.status &&
            ["published", "retracted"].includes(
              entityData.status.toLowerCase(),
            ))) && (
          ...
          >
            This entity is no longer editable. It was locked when it became
            publicly acessible when data associated with it was published.
          </Alert>
        )}
      {entityData && entityData.upload && renderUploadLink(entityData)}
      {entityData && (entityData.doi_url || entityData.registered_doi) && (
        <Typography>
          <a href={entityData.doi_url} target="_blank" rel="noreferrer">
            {entityData.doi_url || entityData.registered_doi}{" "}
          </a>
          <FontAwesomeIcon
            style={{
              color: "rgb(10, 109, 255)",
              fontSize: "0.8em",
              marginLeft: "5px",
            }}
...
export function RenderSubmitModal({
  showSubmitModal,
  ...
  return (
    ...
    >
      <DialogContent>
        ...
        <div>
          {" "}
          Has all data for this dataset been <br />
          1 &#41; validated locally, and <br />2 &#41; uploaded to the globus
          folder?
        </div>
      </DialogContent>
      <DialogActions>
...
export function UserGroupSelectMenu(formValues, menu) {
  ...
  if (formValues.group_name) {
    return (
      <option value={formValues.group_uuid}>{formValues.group_name}</option>
    );
  } else {
    let menuArray = [];
...
export function getEntityRoutePath(entityType, uuid) {
  const normalizedEntityType =
    entityType?.toLowerCase() === "data upload"
      ? "upload"
...
export function combineTypeOptionsComplete(blackList, whitelist) {
  ...
  combinedList.push({
    // @TODO: Find out why Importing Warps this
    donor: "Donor",
    sample: "Sample",
    dataset: "Dataset",
    upload: "Data Upload",
  ...
  if (blackList && blackList.length > 0) {
    ...
    combinedList[0] = Object.fromEntries(
      Object.entries(combinedList[0]).filter(
  ...
  if (whitelist && whitelist.length > 0) {
    ...
    combinedList[0] = Object.fromEntries(
      Object.entries(combinedList[0]).filter(([key]) =>
  ...
  try {
    ...
    typeof error.type === "string"
      ...
      : (errStringMSG = error);
    logger.all.error({
      message: "combineTypeOptionsComplete",
      error_details: { error, errStringMSG },
    });
  }
}
...
export function CombinedEmbeddedEntityOptions() {
  ...
  const menuFilterMap = localStorage.getItem("menuMap")
    ? JSON.parse(localStorage.getItem("menuMap"))
    : {};
  const currentForm = decodeURIComponent(
  ...
  try {
    ...
    return (
      <FormControl size="small" sx={{ width: "100%" }}>
        {embedded && <InputLabel htmlFor="entity_type">Type</InputLabel>}
        {!embedded && (
    ...
    let msg =
      typeof error.type === "string" ? "Error on Organ Assembly" : error;
    logger.all.error({
      message: `CombinedWholeEntityOptions ${msg}`,
      error_details: error,
    });
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ border: `1px solid ${messageColor}` }}>
        <Typography
          sx={{ fontSize: "0.9rem", marginBottom: "10px", marginTop: "10px" }}
        >
          {summary ? summary : defaultSummary}
        </Typography>
        {message && message.length > 0 && typeof message === "object"
          ? message.map(([details, items], index) => (
              <React.Fragment key={index}>
                ...
                <ul
                  style={{
                    ...
                    padding: "20px",
                    background: messageColor + "20",
                    borderRadius: "5px",
                    border: `1px solid ${messageColor + 20}`,
                  }}
                >
                  {Array.isArray(items) &&
                    items.map((item, i) => {
...
export function EntityValidationMessage(props) {
  ...
  if (response?.status === 202)
    message = "This Entity has been accepted for validation.";
  const handleClose = (event, reason) =>
    reason !== "clickaway" && setEValopen(false);
  return (
    <Snackbar
...
export function SnackbarFeedback(props) {
  ...
  return (
    <Snackbar
      ...
      autoHideDuration={
        snackbarController.hide ? snackbarController.hide : 6000
      }
      onClose={closeSnack}
    >
      <Alert
        ...
        sx={{
          width: "100%",
          backgroundColor: snackbarController.color
            ? snackbarController
            : snackbarController.status === "error"
              ? "#f44336"
              : "#4caf50",
        }}
      >
