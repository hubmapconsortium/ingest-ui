...
export const RenderMetadata = (props) => {
  ...
  var handleFileGrab = (e) => {
    ...
    if (newFile && newFile.name.length > 0) {
      ...
      logger.debug("RenderMetadata.handleFileGrab: No Data??", e);
    }
  };
      ...
      <div>
        ...
        {results === "Succeeding" && (
          <Alert className="mb-2" severity="success">
            ...
            No Errors or Failures Detected. Your Data has been accepted and
            import is in progress. Depending on how much metadata was provided,
            this could take several minutes to several hours to import.
          </Alert>
        )}
        {results === "Success" && (
          <Alert className="mb-2" severity="success">
        ...
        {results === "Failure" && (
          <Box className="mb-2" severity="error">
            <FontAwesomeIcon
              ...
              className="mr-1"
            />{" "}
            There were errors encountered when Validating your File:
            ...
            {errorRows.length > 0 &&
              errorRows.map((row, index) => {
                return (
                  ...
                  >
                    <AlertTitle>
                      Error: Row {row.row.toString()} Column{" "}
                      {row.column.toString()}
                    </AlertTitle>
                    {row.error.toString()}
                  </Alert>
  ...
  const handleUploadFile = (newFile) => {
    ...
    try {
      ingest_api_upload_bulk_metadata(
        ...
        .then((resp) => {
          logger.debug("%c◉ resp ", "color:#008000", resp);
          if (resp.status === 202) {
            ...
            setResults("Failure");
            setFailedStep(2);
            var err = resp.error.response.data.error
            ...
            if (!err.includes("row") || !err.includes("[")) {
              ...
              try {
                ...
                logger.all.error({
                  message: "RenderMetadata.handleFileUpload.ln148",
                  error_details: error,
                });
              }
            } else {
              ...
              try {
                ...
                logger.all.error({
                  message: "RenderMetadata.handleFileUpload.ln156",
                  error_details: error,
                });
              }
            }
  ...
  const introText = () => {
    return (
      <>
        ...
        >
          To bulk register {props.type.toLowerCase()} metadata, upload your tsv
          file here. Please refer to the format specified in this{" "}
          <Button href={exampleFile} size="small" download target="_blank ">
            {<FontAwesomeIcon icon={faFileDownload} className="m-1" />}
            Example.tsv{" "}
          </Button>{" "}
          file For further details, please see the Metadata Upload Documentation
          for {props.type}s.
        </Typography>
          ...
          >
            ...
            <Grid size={10} container sx={{ alignItems: "flex-start" }}>
              ...
              >
                ...
                Please do not refresh, close, or leave the page until the
                process is complete.
              </Typography>
            </Grid>
          </Grid>
  ...
  const exampleFile =
    "https://hubmapconsortium.github.io/ingest-validation-tools/sample-" +
    targetFile +
    "/current/";
  ...
  return (
    <div className="row">
      ...
      <div className=" col-sm-10">{introText()}</div>
    </div>
  );
};
