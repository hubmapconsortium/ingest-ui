    ...
    <div className="App">
      ...
      { !isLoading && bannerShow && (
          <div className="alert alert-info" role="alert">
            ...
            <Typography>
              The Data Coordination Working Group (DCWG) and HIVE are in the process of releasing updated schemas for all HuBMAP organs, samples, and assay types. <strong>The HIVE will no longer accept any new data submissions using the previous metadata and directory schemas. Please prepare any new data submissions using the draft next-generation metadata and directory schemas</strong> that are being finalized by the DCWG, and which are linked from <Link to="https://software.docs.hubmapconsortium.org/metadata" target="_blank"> this page</Link>.
            </Typography>
            <Typography>
              The schemas you should use are marked <strong>“use this one”</strong> on the schema pages. You can validate finalized (non-draft) metadata schemas using the <Link to="https://docs.google.com/document/d/1lfgiDGbyO4K4Hz1FMsJjmJd9RdwjShtJqFYNwKpbcZY/" target="_blank"> process outlined here.</Link> If you’re using an assay that is still “draft” and haven’t been in touch with the DCWG, please reach out to them by emailing help@hubmapconsortium.org.
            </Typography>
            <Typography>
              The HIVE will still process any datasets using the previous schemas that were fully submitted and uploaded prior to August 7, or that had submission errors that are currently being worked through.  Please contact  <a href="mailto:help@hubmapconsortium.org ">help@hubmapconsortium.org</a> if you have questions.
            </Typography>
          </div>
      )}
      {isLoading || dtloading &&(
