...
class Neo4jConnection(object):
    ...
    def get_node_by_uuid(self, uuid):
        with self._driver.session() as session:
            node_data = session.write_transaction(self._return_node_by_uuid, uuid)
        return node_data

    @staticmethod
    def _return_node_by_uuid(tx, uuid):
        result = tx.run("MATCH (a {uuid: '" + uuid + "'}) RETURN a.uuid AS uuid, a.label AS label,a.type AS type")
        recordset = []
        for record in result:
            newrecord = { 'uuid' : str(record["uuid"]), 'label' : str(record["label"]), 'type' : str(record["type"]) } 
            recordset.append(newrecord)
        return recordset
    ...
    def does_uuid_exist(self, uuid):
        returnData = self.get_node_by_uuid(uuid)
    ...
    def validate_record(data_record, datatype):
        ...
        for attr in current_required_fields:
...
if __name__ == "__main__":
    ...
    conn.does_uuid_exist('70a43e57-c4fd-4616-ad41-ca8c80d6d827')
    conn.does_uuid_exist('0ce5be9b-8b7f-47e9-a6d9-16a08df05f50')
